...
<div class="px-0 pr-4 sm:pr-6 lg:pr-8 py-6" x-data="filterMenu()" x-init="init()">
    <div class="mb-3 grid grid-cols-1 sm:grid-cols-3 items-center gap-3">
        <div class="justify-self-start flex items-center gap-3">
            ...
            <select id="cal-year-select" class="rounded-md border border-[#5E0F0F] text-[#5E0F0F] bg-white px-2 py-1 text-sm font-medium shadow-sm hover:bg-[#5E0F0F]/5 focus:outline-none focus:ring-2 focus:ring-[#5E0F0F]/30" aria-label="Select Year" title="Select Year">
        ...
        <div class="justify-self-center">
            <div class="inline-flex items-center gap-0">
                ...
                <button id="btn-prev" class="bg-[#5E0F0F] text-white px-[15px] py-[9px] text-sm font-semibold shadow hover:opacity-90 rounded-l-[6px] rounded-r-[0px] cursor-pointer" aria-label="Previous month" title="Previous Month">
                    <i class="fa-solid fa-chevron-left"></i>
                </button>
                <button id="btn-next" class="bg-[#5E0F0F] text-white px-[15px] py-[9px] text-sm font-semibold shadow hover:opacity-90 rounded-r-[6px] rounded-l-[0px] cursor-pointer" aria-label="Next month" title="Next Month">
        ...
        <div class="justify-self-end">
            <div class="relative" @click.outside="open=false">
                <button @click="open=!open" class="inline-flex items-center gap-2 rounded-md bg-[#5E0F0F] px-4 py-2 text-sm font-semibold text-white shadow hover:opacity-90 cursor-pointer">
                    <span>Domains (<span x-text="selected.length"></span>/3)</span>
    ...
    <div id="calendarLegend" class="mt-3 flex flex-wrap items-center gap-3"></div>
</div>
@endsection
