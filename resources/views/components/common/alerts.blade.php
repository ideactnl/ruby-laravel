<div x-data="toastCenter()" x-init="init()"
    ...
    <template x-for="t in toasts" :key="t.id">
        <div x-show="true" @mouseenter="pause(t.id)" @mouseleave="resume(t.id)"
            x-transition:enter="transition ease-out duration-300 transform"
            x-transition:enter-start="translate-x-full opacity-0" x-transition:enter-end="translate-x-0 opacity-100"
            x-transition:leave="transition ease-in duration-200 transform"
            x-transition:leave-start="translate-x-0 opacity-100" x-transition:leave-end="translate-x-full opacity-0"
            :class="wrapperClass(t.type)" class="rounded-lg shadow-lg border border-gray-200 overflow-hidden">

            <!-- Progress bar -->
            <div class="h-1 bg-gray-100">
                <div class="h-full transition-all duration-100 ease-linear" :class="barClass(t.type)"
    ...
    function toastCenter() {
        return {
            ...
            resume(id) {
                const t = this.toasts.find(x => x.id === id);
                if (t && t.paused) {
                    const pausedDuration = Date.now() - t.pauseStart;
                    t.created += pausedDuration;
                    t.paused = false;
                    t.pauseStart = null;
                }
            },
