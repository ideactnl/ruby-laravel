<nav class="sticky top-0 z-50 bg-white/95 backdrop-blur border-t-0 border-[#fff]">
    <div class="mx-auto w-full px-5 md:px-8 border-b border-[#ebebeb]">
        <div class="flex h-20 items-center justify-between border-b border-gray-100/70">
            <div class="flex min-w-0 items-center gap-4">
                <div class="truncate">
                    <h1 class="truncate text-lg font-bold tracking-wide text-[#000] sm:text-2xl">@yield('navbar_title', 'RUBY - MEDICAL SPECIALIST')</h1>
            ...
            <div class="flex items-center gap-2 sm:gap-3">
                ...
                @if(session('medical_specialist_id'))
                    ...
                    <div class="relative" x-data="{open:false}" @click.outside="open=false">
                        <button @click="open=!open" class="inline-flex h-9 w-9 items-center justify-center rounded-full bg-[#5E0F0F] text-white focus:outline-none cursor-pointer" aria-label="Medical Specialist Account">
                            <span>{{ strtoupper(substr($participant->registration_number ?? 'M', 0, 1)) }}</span>
                        </button>
                        <div x-show="open" x-transition class="absolute right-0 mt-2 w-56 rounded-md border border-gray-200 bg-white shadow-lg py-1 z-50" style="display:none">
                            ...
                            <form method="POST" action="{{ route('medical-specialist.logout') }}" class="block">
                                @csrf
                ...
                @else
                    <a href="{{ route('medical-specialist.login') }}" class="inline-flex items-center rounded-md bg-[#5E0F0F] px-4 py-2 text-sm font-semibold text-white shadow hover:opacity-90">Login</a>
                @endif
            </div>
        </div>
    </div>
