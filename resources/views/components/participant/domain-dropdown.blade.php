...
<div {{ $attributes->merge(['class' => 'relative flex-shrink-0 ' . $className]) }} @click.outside="open=false" x-cloak>
    <button
    ...
    <div x-cloak x-show="open" x-transition
        ...
        <template x-for="opt in options" :key="opt.value">
            <label class="flex cursor-pointer items-center gap-2 rounded px-2 py-2 text-sm transition-all duration-200"
                :class="{
                    'hover:bg-gray-50': !(!selected.includes(opt.value) && selected.length >= 3),
                    'opacity-50 cursor-not-allowed': !selected.includes(opt.value) && selected.length >=
                        3,
                ...
                <div class="w-5 h-5 flex items-center justify-center rounded border border-gray-300 cursor-pointer transition-all duration-300 ease-in-out transform hover:scale-105"
                    ...
                    <svg x-show="selected.includes(opt.value)" x-transition:enter="transition ease-out duration-200"
                        x-transition:enter-start="opacity-0 scale-75" x-transition:enter-end="opacity-100 scale-100"
                        x-transition:leave="transition ease-in duration-150"
                        x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-75"
                        xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 text-white" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor">
                ...
                <span x-text="opt.label" class="transition-all duration-300 ease-in-out"
                    :class="{
                        ...
                        'text-gray-700': !selected.includes(opt.value) && !(!selected.includes(opt
                                .value) &&
                            selected.length >= 3)
                    }">
                </span>

                <input type="checkbox" class="hidden" :value="opt.value" :checked="selected.includes(opt.value)"
                    @change="toggle(opt.value)" :disabled="!selected.includes(opt.value) && selected.length >= 3" />
            </label>
        </template>
